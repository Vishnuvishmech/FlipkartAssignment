...
public class YamlPage {
    ...
    public YamlPage(String platform, String root, String pageFile) throws Exception{
        ...
        if(platform.equalsIgnoreCase("android")){
            Map<String, Map> elements = (Map<String, Map>)page.get("androidelements");
            for(int index=0; index<elements.size(); index++){
                String key = (String)elements.keySet().toArray()[index];
                yamlElements.put(key, new YamlElement(key, elements, timeout));
            }
        }else if(platform.equalsIgnoreCase("ios")) {
            Map<String, Map> elements = (Map<String, Map>)page.get("ioselements");
            for(int index=0; index<elements.size(); index++){
                String key = (String)elements.keySet().toArray()[index];
                yamlElements.put(key, new YamlElement(key, elements, timeout));
            }
